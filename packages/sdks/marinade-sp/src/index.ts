...
import {MarinadeClientParams, Utils} from "./utils";
import {
    MarinadeUtils,
    Provider,
    type Wallet,
...
export class MarinadeClient extends BeamInterface<MarinadeBeam, StateAccount> {
    ...
    public static async get(
        ...
        const marinadeClientParamsPromise = Utils.getMarinadeClientParams(provider, programId, stateAddress);

        const marinadeClientParams = await marinadeClientParamsPromise;
        const sunriseClient = await sunriseClientPromise;

        return new MarinadeClient(program, stateAddress, account, marinadeClientParams, sunriseClient)
    }

    /** Query on-chain data for the most recent account state. */
    public refresh(): Promise<this> {
        return MarinadeClient.get(this.provider, this.stateAddress, this.program.programId) as Promise<this>;
    }
